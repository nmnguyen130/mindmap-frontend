...
import { databaseService, MindMapRow, MindMapNodeRow, ConnectionRow } from '@/services/database'
import { MindMap, MindMapNode } from '@/stores/mindmaps'

// Convert database rows to store types
function mindMapFromRow(row: MindMapRow, nodes: MindMapNodeRow[], connections: ConnectionRow[]): MindMap {
...
export function useMindMap(id: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return mindMapFromRow(fullMindMap.mindMap, fullMindMap.nodes, fullMindMap.connections)
    },
    staleTime: 1000 * 60 * 5, // 5 minutes
    enabled: !!id,
  })
...
export function useCreateMindMap() {
  ...
  return useMutation({
    mutationFn: async (mindMap: Omit<MindMap, 'id' | 'createdAt' | 'updatedAt'>) => {
...
export function useUpdateMindMap() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['mindmaps'] })
      queryClient.invalidateQueries({ queryKey: ['mindmap', data.id] })
    },
  })
}
...
export function useUpdateNode() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['mindmap', data.mindMapId] })
    },
  })
}
...
export function useCreateNode() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['mindmap', variables.mindMapId] })
    },
  })
}
...
export function useDeleteNode() {
  ...
  return useMutation({
    mutationFn: async ({ mindMapId, nodeId }: { mindMapId: string; nodeId: string }) => {
      await databaseService.deleteNode(nodeId)
...
export function useDeleteConnection() {
  ...
  return useMutation({
    mutationFn: async ({ mindMapId, connectionId }: { mindMapId: string; connectionId: string }) => {
