...
class DatabaseService {
  ...
  // Mind Map operations
  async createMindMap(mindMap: Omit<MindMapRow, 'created_at' | 'updated_at'>): Promise<string> {
    const db = await this.initialize()
    const result = await db.runAsync(
      'INSERT INTO mindmaps (id, title) VALUES (?, ?)',
      [mindMap.id, mindMap.title]
    )
    return result.lastInsertRowId?.toString() || mindMap.id
  }
  ...
  async updateMindMap(id: string, updates: Partial<Pick<MindMapRow, 'title'>>): Promise<void> {
    const db = await this.initialize();
  ...
  // Node operations
  async createNode(node: Omit<MindMapNodeRow, 'created_at' | 'updated_at'>): Promise<string> {
    const db = await this.initialize()
    const result = await db.runAsync(
    ...
    return result.lastInsertRowId?.toString() || node.id
  }
  ...
  async updateNode(id: string, updates: Partial<Pick<MindMapNodeRow, 'text' | 'position_x' | 'position_y'>>): Promise<void> {
    const db = await this.initialize();
    const setParts: string[] = [];
    const values: any[] = [];
  ...
  async createConnection(connection: Omit<ConnectionRow, 'created_at'>): Promise<string> {
    const db = await this.initialize()
    const result = await db.runAsync(
      'INSERT INTO connections (id, mindmap_id, from_node_id, to_node_id) VALUES (?, ?, ?, ?)',
      [connection.id, connection.mindmap_id, connection.from_node_id, connection.to_node_id]
    )
    return result.lastInsertRowId?.toString() || connection.id
  }
